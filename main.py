...
# Define paths 
base_dir = "/media/azzakhns/Storage/"
out_dir = '../theardia_baseline_exp/'
...
partitions_csv_path = os.path.join(base_dir, "partitions.csv")
key_file = os.path.join(base_dir, "partitions2.csv")  ## corresponding new ids of the old ids after processing the old version of the data
...
if os.path.exists(os.path.join(save_dir, json_name)):
    loader.load_from_json()

else:
    print("JSON file not found. Generating new data.")
    loader.index_data()
    loader.add_annotations_for_users()
    #loader.add_google_ASR_transcriptions()
    loader.save_to_json()
...
# Labels Regression Section
def exp_main_labels_reg(train_data, dev_data, test_data, feat_title, feat_params, feat_funcs, model_title, model_params, hyper_title, hyper_params, target_label):
    exp_name = "data_" + audio_source + "_labels_reg_MLP"
    ...
    compute_cost = MSEWrapper()
    exp = Experimenter_lab(exp_dir=exp_dir)
    ...
    data_train_sb = setup_partition_reg(train_data, target_label=target_label, seed=hyper_params["seed"])
    data_dev_sb = setup_partition_reg(dev_data, target_label=target_label, seed=hyper_params["seed"])
    data_test_sb = setup_partition_reg(test_data, target_label=target_label, seed=hyper_params["seed"])
...
def exp_labels_reg_loops(train_data, dev_data, test_data, device="cpu"):
    seed = 0
    feats = {
    ...
    models = {
        "mlp": {"model": myMLP, "params": {"output_size": 1, "hidden_size": "feat_size/2"}},
    }
    hypers = {
        ...
        for feat_title, feat_params in feats.items():
            for model_title, model_params in models.items():
                for hyper_title, hyper_params in hypers.items():
                    exp_main_labels_reg(train_data, dev_data, test_data, feat_title, feat_params, feat_funcs, model_title, model_params, hyper_title, hyper_params, target_label)

print('labels regression experiments...')
exp_labels_reg_loops(train_dict, dev_dict, test_dict)
...
def exp_labels_class_loops(train_data, dev_data, test_data, device="cpu"):
    seed = 0
    feats = {
        "tfidf": {"corpus": [datum["trs"] for datum in list(train_data.values())], "seed": seed, "trs_tar": "trs"},
        "tfidf_google": {"corpus": [datum["trs_google"] for datum in list(train_data.values())], "seed": seed, "trs_tar": "trs_google"},
...
def exp_dim_reg_loops(train_data, dev_data, test_data, device="cpu"):
    seed = 0
    feats = {
        "CLIP_csv": {"csv_dir": "/data/safaa/experiements_theradia/Data/Theradia/Features/clip", "feats_size": 512},
    }
...
print('dimensions summaries regression')
exp_dim_reg_loops(train_dict, dev_dict, test_dict)

# Continuous Dimensions Regression Section
def exp_fusion_cont_reg(train_data, dev_data, test_data, feat_titles, feat_params, feat_funcs, model_title, model_params, hyper_title, hyper_params, target_label, gs_method):
